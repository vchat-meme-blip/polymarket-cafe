  ...
  private async get<T = any>(endpoint: string, params: Record<string, any> = {}): Promise<T | null> {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        console.error(`[SolscanService] Request FAILED <- ${resp.status} ${resp.statusText}. URL: ${url.toString()}, Response: ${errorText}`);
        return null;
      }
  ...
  async fetchTickerTokens(): Promise<any[]> {
    try {
      const [trending, bonding] = await Promise.all([
        ...
        this.fetchLaunchpadTokens(20, 'pumpfun')
      ]);

      const allTokens = [...(trending || []), ...(bonding || [])];
