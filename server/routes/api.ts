...
router.get('/leaderboard/pnl', async (req, res) => {
    try {
        const data = await leaderboardService.getPnlLeaderboard();
        res.json(data);
    } catch (error) {
        console.error('[API] Error fetching PNL leaderboard:', error);
        res.status(500).json({ message: 'Failed to fetch PNL leaderboard.' });
    }
});

router.get('/leaderboard/intel', async (req, res) => {
    try {
        const data = await leaderboardService.getIntelLeaderboard();
        res.json(data);
    } catch (error) {
        console.error('[API] Error fetching Intel leaderboard:', error);
        res.status(500).json({ message: 'Failed to fetch Intel leaderboard.' });
    }
});
...
router.post('/markets/by-ids', async (req, res) => {
    ...
    try {
        const { markets: allLiveMarkets } = await polymarketService.getLiveMarkets(500);
