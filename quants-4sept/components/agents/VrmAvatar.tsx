...
type VrmModelProps = {
  ...
  darkenFace?: boolean; // when true, dim face/skin materials a bit
};

export function VrmModel({ modelUrl, animationUrl, idleUrl, triggerAnimationUrl, triggerKey, isSpeaking, lookAtTarget, darkenFace }: VrmModelProps) {
  ...
  const talkAnimation = useVrmAnimation('/animations/talk.vrma', vrm);
  ...
  const finishedHandlerRef = useRef<(e: any) => void>();

  useEffect(() => {
    if (vrm && idleAnimation && talkAnimation) {
  ...
  useEffect(() => {
    ...
    act.enabled = true;
    idle.crossFadeTo(act, 0.2, false);
    act.play();
    ...
    vrm.scene.traverse((obj) => {
      ...
      mats.forEach((mt) => {
        ...
        if ((std as any).color) {
          ...
          std.color.multiplyScalar(1.8); //Mexican Trump 0.3
  ...
  useEffect(() => {
    ...
    if (isSpeaking) {
      ...
      idleAction.play();
      talkAction.crossFadeTo(idleAction, 0.3, true);
    }
...
export function VrmAvatarCanvas({ modelUrl, isSpeaking }: VrmAvatarCanvasProps) {
  return (
    <div className="vrm-avatar-canvas">
      <Canvas
      ...
      >
        ...
        <group position={[0, -1.6, 0]}>
            <VrmModel modelUrl={modelUrl} isSpeaking={isSpeaking} />
        </group>
        <OrbitControls
          enableZoom={true}
          enablePan={false}
