...
const OPENAI_KEY_PREFIXES = ['sk-', 'rk-'];

const looksLikeOpenAIKey = (key: string): boolean =>
  OPENAI_KEY_PREFIXES.some(prefix => key.startsWith(prefix));

const toOpenAIKey = (raw: unknown): string | null => {
  if (typeof raw !== 'string') return null;
  return looksLikeOpenAIKey(raw) ? raw : null;
};
...
class ApiKeyService {
  ...
  constructor() {
    ...
    try {
      ...
      usersWithKeys.forEach(u => {
        const normalized = toOpenAIKey(u.userApiKey);
        if (normalized) {
          keySet.add(normalized);
        } else if (typeof u.userApiKey === 'string') {
          const suffix = u.userApiKey.slice(-4);
          console.warn(
            `[ApiKeyService] Ignoring non-OpenAI user key ending in ...${suffix}.`
          );
        }
      });

      // 2. Scan environment variables for all OPENAI_API_KEYs
      for (const key in process.env) {
        if (key.startsWith('OPENAI_API_KEY') && process.env[key]) {
          const envKey = toOpenAIKey(process.env[key]);
          if (envKey) {
            keySet.add(envKey);
          } else {
            console.warn(
              `[ApiKeyService] Ignoring env var ${key} - value does not look like an OpenAI key.`
            );
          }
        }
      }
  ...
  public async getKeyForAgent(agentId: string): Promise<string | null> {
    ...
    if (!agent) {
        ...
        return toOpenAIKey(process.env.OPENAI_API_KEY);
    }
    ...
    if (isUserAgent) {
        ...
        if (owner) {
            const ownerKey = toOpenAIKey(owner.userApiKey);
            if (ownerKey) {
            const unlockTime = this.cooldowns.get(ownerKey);
            if (!unlockTime || now > unlockTime) {
                // Owner's key is available and not on cooldown
                if (unlockTime) this.cooldowns.delete(ownerKey);
            ...
            } else if (typeof owner.userApiKey === 'string') {
            console.warn(
              `[ApiKeyService] Ignoring non-OpenAI owner key for agent ${agent.name}. Falling back to pool.`
            );
            }
        } else {
    ...
    const fallbackKey = toOpenAIKey(process.env.OPENAI_API_KEY);

    if (!fallbackKey) {
      console.warn('[ApiKeyService] OPENAI_API_KEY fallback is missing or invalid.');
    }

    return keyFromPool || fallbackKey;
  }
}
