...
const HolographicOffer = ({ offer }: { offer: Offer }) => {
  ...
  let text = '';
  if (offer.type === 'intel') {
      text = `INTEL ON $${offer.token}\n@ ${offer.price} BOX`;
  } else if (offer.type === 'token') {
      text = `${offer.action} ${offer.quantity} $${offer.token}\n@ ${offer.price_per_token} BOX`;
  }
...
function RoomScene({ room }: RoomSceneProps) {
    ...
    useEffect(() => {
        if (room.activeOffer) {
            const newOfferKey = `${room.activeOffer.type}-${room.activeOffer.token}-${(room.activeOffer as any).price || (room.activeOffer as any).price_per_token}-${Date.now()}`;
