...
const OperationsTab = ({ agent, onUpdate }: { agent: Partial<Agent>, onUpdate: (updates: Partial<Agent>) => void }) => {
    return (
        <div className={styles.operationsContainer}>
            ...
            <p>Set rules for how this agent operates on its own in the Café.</p>
            <div className={styles.operationsForm}>
                <label>
                    ...
                    />
                    <p className={styles.stepHint}>If filled, this agent will only visit these specific rooms to buy intel. Leave blank to allow random roaming.</p>
                </label>
                <label>
                    ...
                    />
                    <p className={styles.stepHint}>Define when this agent should be active if they are hosting an owned room. (Parsing is very basic for now).</p>
                </label>
            </div>
        </div>
    );
...
const PortfolioTab = ({ agent }: { agent: Partial<Agent> }) => {
    // FIX: Safely access the `portfolio` property.
        ...
        <div className={styles.portfolioContainer}>
            <div className={styles.portfolioSummary}>
                <div className={styles.balanceCard}>
                    ...
                    {/* FIX: Safely access the `boxBalance` property. */}
                    <span className={styles.balanceValue}>{(agent.boxBalance || 0).toLocaleString()}</span>
                </div>
            </div>
