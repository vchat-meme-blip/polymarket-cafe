...
export default function ManageRoomModal() {
    const { closeManageRoomModal, addToast, openShareRoomModal } = useUI();
    ...
    useEffect(() => {
        // Only set the initial form data when the modal opens or the room data loads.
        // This prevents background updates from overwriting user input.
        if (room) {
            setFormData({
                name: room.name || '',
    ...
    }, [room?.id]); // Depend only on the room ID to set initial state
